...
9. Application updates never replace business data. Every schema migration requires a verified pre-migration backup and stops safely on failure. Automatic binary/schema rollback is never assumed; recovery uses a corrective release or a documented restore procedure.
